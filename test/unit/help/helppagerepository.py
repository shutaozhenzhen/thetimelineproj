...
class describe_help_page_repoistory(UnitTestCase):

    def setUp(self):
        UnitTestCase.setUp(self)
        self.repository = HelpPageRepository(HOME_PAGE_ID, HELP_RESOURCES_ROOT_DIR, "page:")
    ...
    def test_pages_are_defined_by_id_header_body_and_related_pages(self):
        self.repository.install_page("id", "header", "body", [])

    def test_pages_can_be_retrieved_by_page_id(self):
        self.repository.install_page("id", "header", "body", [])
        page = self.repository.get_page("id")
    ...
    def test_pages_has_id_header_body_and_related_pages(self):
        ...
        self.assertEqual("header", page.header)
        self.assertEqual("body", page.body)
        self.assertEqual([], page.related_pages)

    def test_pages_can_be_found_by_text_search_in_header(self):
        self.repository.install_page("id", "header", "body", [])
        pages = self.repository._find_pages("header")
        self.assertEqual(1, len(pages))
        self.assertEqual("header", pages[0].header)
...
class describe_help_page(UnitTestCase):

    def setUp(self):
        UnitTestCase.setUp(self)
        self.repository = HelpPageRepository(HOME_PAGE_ID, HELP_RESOURCES_ROOT_DIR, "page:")
        self.help_page = HelpPage(self.repository, "id", "header", "body", [])
    ...
    def test_has_id_header_body_and_related_pages(self):
        self.assertEqual("id", self.help_page.page_id)
        self.assertEqual("header", self.help_page.header)
        self.assertEqual("body", self.help_page.body)
        self.assertEqual([], self.help_page.related_pages)

    def test_can_be_rendered_to_html(self):
        self.assertEqual("<h1>header</h1><p>body</p>", self.help_page.render_to_html())

    def test_can_be_rendered_to_html_with_related_pages(self):
        self.repository.install_page("foo", "header", "body", [])
        self.repository.install_page("bar", "header", "body", [])
        self.help_page = HelpPage(self.repository, "id", "header", "body", ["foo", "bar"])
    ...
    def test_can_be_rendered_to_html_with_related_pages_not_found(self):
        self.help_page = HelpPage(self.repository, "id", "header", "body", ["foo", "bar"])
        self.assertEqual('<h1>header</h1><p>body</p><h2>#Related pages#</h2><ul></ul>', 
                         self.help_page.render_to_html())
