...
class PharaonicDatePickerController(humblewx.Controller):
    ...
    def on_init(self, date_formatter, date_modifier, on_change):
    ...
    def on_key_up(self):
        try:
            date = self.get_pharaonic_date()
        except ValueError:
            pass
        else:
            self.set_pharaonic_date(self._get_incrementer()(date))
    ...
    def on_key_down(self):
        try:
            ...
            self.set_pharaonic_date(self._get_decrementer()(date))

    def set_pharaonic_date(self, date):
        (formatted_date, is_bc) = self._date_formatter.format(date)
        self.view.SetText(formatted_date)
        self.view.SetIsBc(is_bc)

    def get_pharaonic_date(self):
