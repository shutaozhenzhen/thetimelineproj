...
class ShortcutsEditorDialog(ShortcutsGuiCreator):
    ...
    def display_ack_popup_window(self, text):
        def calculate_ack_popup_window_position():
            pos = [a + b for a, b in zip(self.GetPosition(), self.btn_apply.GetPosition())]
            return list(pos)
        GREEN = "#D3F4B8"
        win = PopupTextWindow(self, GREEN, text, timeout=1200)
        win.Position(calculate_ack_popup_window_position(), (-1, -1))
        win.Popup()
