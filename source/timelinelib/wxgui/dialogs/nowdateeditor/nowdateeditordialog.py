...
from timelinelib.wxgui.dialogs.nowdateeditor.nowdateeditorcontroller import NowDateEditorController
...
class NowDateEditorDialog(wx.Dialog):
    ...
    def __init__(self, parent, db, event):
        wx.Dialog.__init__(self, parent, title=_("Now"))
        self._create_gui(db.get_time_type().get_duplicate_functions())
        self.controller = NowDateEditorController(self, db, event)
...
def open_now_date_editor_dialog_for_event(parent, db, handle_db_error, event):
    def create_dialog():
        return NowDateEditorDialog(parent, db, event)
