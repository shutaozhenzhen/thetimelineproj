...
def a_time_period():
    ...
    return TimePeriod(CopticDateTime(year, month, day, 0, 0, 0).to_time(),
...
def an_event_with(human_start_time=None, human_end_time=None, time=ANY_TIME,
    ...
    if human_start_time and human_end_time:
        start = human_time_to_coptic(human_start_time)
        end = human_time_to_coptic(human_end_time)
    else:
...
def a_container(name, category, sub_events):
    ...
    start = human_time_to_coptic(ANY_TIME)
    end = human_time_to_coptic(ANY_TIME)
...
def a_coptic_era():
    ...
    return a_coptic_era_with()


def a_coptic_era_with(start=None, end=None, time=ANY_TIME, name="foo",
                         color=(128, 128, 128), time_type=CopticTimeType(),
                         ends_today=False):
    """Create an :doc:`Era <timelinelib_canvas_data_era>` object."""
    if start and end:
        start = human_time_to_coptic(start)
...
EVENT_MODIFIERS = [
    ...
    ("change time period", lambda event:
        event.set_time_period(event.get_time_period().move_delta(CopticDelta.from_days(1)))),
...
ERA_MODIFIERS = [
    ...
    ("change time period", lambda era: era.set_time_period(era.get_time_period().move_delta(CopticDelta.from_days(1)))),
...
TIME_MODIFIERS = [
    ("add", lambda time: time + CopticDelta(1)),
]
